...
static void device_capture_touch_report(unsigned int count)
{
	...
	case REPORT_TOUCH:
		...
		g_device_hcd->report.data_length = 0;
...
static int device_capture_touch_report_config(unsigned int count)
{
	...
	if (g_device_hcd->raw_mode) {
		...
		buf_size = g_device_hcd->out.buf_size - 3;
	} else {
		size = count - 1;
		...
		buf_size = g_device_hcd->out.buf_size - 1;
	}
...
static int device_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->rmidev_major_num) {
		dev_num = MKDEV(tcm_hcd->rmidev_major_num, 0);
		...
		tcm_hcd->rmidev_major_num = MAJOR(dev_num);
	}
