...
#define BT_TX_POWER_TSSI	(0x7C05B320)
...
static int btmtk_read_register(struct btmtk_dev *bdev, u32 reg, u32 *val)
{
	int ret = 0;
...
static int btmtk_write_register(struct btmtk_dev *bdev, u32 reg, u32 *val)
{
        int ret = 0;

	u8 cmd[] = { 0x01, 0x6F, 0xFC, 0x14,
			0x01, 0x08, 0x10, 0x00,
			0x01, 0x01, 0x00, 0x01,
			0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00,
			0xFF, 0xFF, 0xFF, 0xFF };
	u8 event[] = { 0x04, 0xE4, 0x08,
			0x02, 0x08, 0x04, 0x00,
			0x00, 0x00, 0x00, 0x01 };

        BTMTK_INFO("%s: write cr 0x%08x, value=0x%08x", __func__, reg, *val);
...
void bthost_read_tx_power(void)
{
	struct btmtk_uart_dev *cif_dev = NULL;
	u32 val;
	int err = 0;
	...
	/* check own state and read CR */
	if (cif_dev->own_state == BTMTK_DRV_OWN) {
		btmtk_read_register(g_sbdev, BT_TX_POWER_TSSI, &val);
		BTMTK_WARN("%s:drv own, Read CR 0x7C05B320:[0x%08x], own_state[%d]", __func__, val, cif_dev->own_state);

		if ((val & 0xFF) == 0x01) {
			conn_dbg_add_log(CONN_DBG_LOG_TYPE_HW_ERR, "[bt] RF0 10 packets TX power too large in LV9 TSSI offset...");
		}

                if (((val >> 16) & 0xFF) == 0x01) {
                        conn_dbg_add_log(CONN_DBG_LOG_TYPE_HW_ERR, "[bt] RF1 10 packets TX power too large in LV9 TSSI offset...");
                }

		val = 0;
		err = btmtk_write_register(g_sbdev, BT_TX_POWER_TSSI, &val);
		if (err < 0) {
			BTMTK_ERR("btmtk_write_register failed!");
			return;
		}
	} else {
		BTMTK_WARN("%s:not in drv own, own_state[%d]", __func__, cif_dev->own_state);
	}
