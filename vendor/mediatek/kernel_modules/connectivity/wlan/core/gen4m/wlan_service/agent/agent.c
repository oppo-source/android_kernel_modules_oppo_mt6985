...
extern int g_SubsysRstTotalCnt;
extern u_int8_t g_SubsysRstCnt;
int g_last_subsys_rst_total_cnt;
int g_last_subsys_rst_cnt;
u_int32 g_first_tx_flag = 1;
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	g_last_subsys_rst_total_cnt = g_SubsysRstTotalCnt;
	g_last_subsys_rst_cnt = g_SubsysRstCnt;
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	if (g_first_tx_flag) {
		...
		if (g_last_subsys_rst_total_cnt != g_SubsysRstTotalCnt) {
			SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
				("%s:total:last=%d, current=%d\n", __func__, g_last_subsys_rst_total_cnt, g_SubsysRstTotalCnt));
			return SERV_STATUS_AGENT_FAIL;
		}

		if (g_last_subsys_rst_cnt != g_SubsysRstCnt) {
			SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
				("%s: last=%d, current=%d\n", __func__, g_last_subsys_rst_cnt, g_SubsysRstCnt));
			return SERV_STATUS_AGENT_FAIL;
		}
	}
